...
このプロジェクトは、既存の多数のffmpeg用バッチファイルを置き換えるためのPython/Tkinterアプリです。
目的は、静止画像、SRT字幕、MP3/WAV音源を合成し、静止画背景に字幕を焼き込んだMP4動画を作ることです。

現行コードは `subtitle_image_mp4_maker.py` です。バージョン表記は `1.0.0` です。
ユーザー環境で起動・基本動作は確認済みです。
...
- SRT読み込み時も、同名の音源と `<SRT名>_title.png` を自動検出する。
...
- Alignmentは `6（中央上）` / `10（中央）` / `2（中央下）` の三択UI。
...
- PyInstaller用バッチに `--collect-all tkinterdnd2` と `--hidden-import tkinterdnd2.TkinterDnD` を入れている。
...
`tkinterdnd2` はPythonモジュールとTcl/Tk側データの両方を必要とするため、PyInstallerでは `--collect-all tkinterdnd2` と `--hidden-import tkinterdnd2.TkinterDnD` を指定します。現行バッチにはこの指定を入れています。
...
- `sibling_audio_for_srt()`: SRTと同名音源の自動検出。
- `sibling_title_image_for_path()`: 音源/SRTと同じstemの `_title.png` 自動検出。
